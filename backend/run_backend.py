...
# Serve React build as static assets
app = Flask(
    __name__,
...
# ---------------------------------------------------------------------------
# React Single-Page-App fallback (serve build)
# ---------------------------------------------------------------------------
@app.route('/', defaults={'path': ''})
@app.route('/<path:path>')
def serve_react(path):
    """Serve React build files; fallback to index.html for SPA routes."""
    try:
        target = BUILD_DIR / path
        if path and target.exists():
            # Serve actual asset (js/css/media)
            return send_from_directory(str(BUILD_DIR), path)
        # Fallback to index.html for client-side routing
        return send_from_directory(str(BUILD_DIR), 'index.html')
    except Exception:
        # If build doesn't exist yet, return a simple hint
        return jsonify({
            'message': 'Frontend build not found. Ensure `npm run build` has run and build/ exists.'
        }), 404
